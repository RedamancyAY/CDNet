...
from toolkits.data import WeightedRandomSamplerDDP
import torch
...
def dist_train(gpu, args):
    ...
    cuttings_size = args.size_cuttings
    images_size = args.size_images
    ...
    transformer1 = utils.get_transformer(face_policy=face_policy, patch_size=images_size,
                                         net_normalizer=model_t.get_normalizer(), train=True)
    transformer2 = utils.get_transformer(face_policy=face_policy, patch_size=cuttings_size,
                                         net_normalizer=model_t.get_normalizer(), train=True)
    ...
    train_dataset = FrameFaceDataset(root=ffpp_faces_dir,
                                     ...
                                     size_images=images_size,
                                     size_cuttings=cuttings_size
                                     )
    ...
    # val_dataset_2 = FrameFaceDataset(root=celebdf_faces_dir,
    #                                  df=dfs_)
    w1 = weightsGen(dfs_train)
    w2 = weightsGen(dfs_val)
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset,
                                                                    ...
                                                                    rank=rank)
    # train_sampler = WeightedRandomSamplerDDP(
    #     train_dataset, w1, world_size, rank, len(train_dataset))

    val_sampler = torch.utils.data.distributed.DistributedSampler(val_dataset_1,
                                                                  num_replicas=world_size,
                                                                  rank=rank)
    # val_sampler = WeightedRandomSamplerDDP(
    #     val_dataset_1, w2, world_size, rank, len(val_dataset_1))
    ...
    val_loader = torch.utils.data.DataLoader(val_dataset_1,
                                             num_workers=num_workers,
                                             batch_size=batch_size,
                                             pin_memory=True,
                                             sampler=val_sampler)
    ...
    while epoch != epoch_run:
        ...
        for train_batch in tqdm(train_loader, desc='Epoch {:03d} '.format(epoch), leave=False,
            ...
            train_batch_loss, pred = batch_forward1(
        ...
        #     acc_total2 = acc_total2+acc2*train_batch_num
        #     # *运用优化器
        ...
        #     # *对模型进行验证
            if iteration > 0 and (iteration % validation_interval == 0):
                ...
                val_loss = validation_routine(
                    model_t, val_loader, 'val', iteration, tb)
                tb.flush()
...
def batch_forward1(model: nn.Module, data: list, labels: torch.Tensor, paths: list[Path]):

    # correct,total = 0,0
    # data1 = data[0].float().cuda(non_blocking=True)
    # data2 = data[1].float().cuda(non_blocking=True)
    data1 = data.float().cuda(non_blocking=True)
...
def clusterloss_1(fea: torch.Tensor, labels_map: torch.Tensor):

    loss = 0
    fea = (nn.functional.normalize(fea)+1)/2
    T_P = torch.where(labels_map == 1)
    F_P = torch.where(labels_map == 0)
    fea_T = fea[T_P]
    fea_F = fea[F_P]
    l_T = fea_T.size(0)
    l_F = fea_F.size(0)
    ...
    fea1 = fea_F.transpose(0, 1)
    sim = torch.mm(fea_T, fea1)
    sim1 = torch.mm(fea_T, fea_T.transpose(0, 1))
    sim2 = torch.mm(fea_F, fea_F.transpose(0, 1))

    loss1 = torch.sum(sim)
    loss2 = (torch.sum(sim1)-torch.sum(torch.diag(sim1)))
    loss3 = (torch.sum(sim2)-torch.sum(torch.diag(sim2)))
    loss = (loss1/(l_T*l_F))/((loss2/(l_T*l_T)) +
                              (loss3/(l_F*l_F)))
    return loss
...
def validation_v1(net, val_loader_1, iteration, tb, tag):
    ...
    for val_data in tqdm(val_loader_1, desc='Validation', leave=False, total=len(val_loader_1)):
        ...
        with torch.no_grad():
            val_acc1, val_acc2, val_loss = batch_forward_val(
...
def validation_routine(net, val_loader, tag: str, iteration, tb, loader_len_norm: int = None):
    ...
    for val_data in tqdm(val_loader, desc='Validation', leave=False, total=len(val_loader)):
        ...
        with torch.no_grad():
            val_batch_loss, val_pred = batch_forward1(
                net, batch_data, batch_labels, batch_paths)
        ...
        # seq = val_batch_pred >= 0.5
        # val_labels_list1.append(val_batch_labels[0:32].cpu())
        # val_pred_list1.append(seq[0:32].cpu())

        # val_labels_list2.append(val_batch_labels[32:48].cpu())
        # val_pred_list2.append(seq[32:48].cpu())

        # val_labels_list3.append(val_batch_labels[48:64].cpu())
        # val_pred_list3.append(seq[48:64].cpu())
    ...
    # val_labels1 = np.concatenate(val_labels_list1).flatten()
    # val_pred1 = np.concatenate(val_pred_list1).flatten()

    # val_labels2 = np.concatenate(val_labels_list2).flatten()
    # val_pred2 = np.concatenate(val_pred_list2).flatten()

    # val_labels3 = np.concatenate(val_labels_list3).flatten()
    # val_pred3 = np.concatenate(val_pred_list3).flatten()

    # acc1 = accuracy_score(val_labels1, val_pred1)
    # acc2 = accuracy_score(val_labels2, val_pred2)
    # acc3 = accuracy_score(val_labels3, val_pred3)

    # tb.add_scalar('{}/acc_TF'.format(tag), acc1, iteration)
    # tb.add_scalar('{}/acc_TT'.format(tag), acc2, iteration)
    # tb.add_scalar('{}/acc_FF'.format(tag), acc3, iteration)

    return val_loss


def weightsGen(df):
    print("Generate weights for sample balance")

    l_f = len(df[df['label'] == 1])
    l_r = len(df[df['label'] == 0])
    weights = [0.25]*l_f+[0.75]*l_r
    return weights


def main():

    args = config.config_3()
