...
def dist_train(gpu, args):
    ...
    model_t_class = getattr(models1, args.net_t)

    model_t = model_t_class()
    transformer = utils.get_transformer(face_policy=face_policy, patch_size=32,
    ...
    while epoch != epoch_run:
        ...
        for train_batch in tqdm(train_loader, desc='Epoch {:03d} '.format(epoch), leave=False,
            ...
            train_batch_loss, acc1, acc2 = batch_forward(
                model_t, batch_data, batch_labels, batch_df, optimizer.param_groups[0]['lr'], iteration)

            # _, seq = torch.min(train_batch_pred, 1)
            # seq = train_batch_pred >= 0.5

            # train_labels_list.append(train_batch_labels.cpu())
            # train_pred_list.append(seq.cpu())
            ...
            if iteration > 0 and (iteration % log_interval == 0):
                ...
                tb.add_scalar('train/acc1', acc_total1, iteration)
...
def batch_forward(model: nn.Module, data: torch.Tensor, labels: torch.Tensor, paths: list[Path], lr, iteration):
    ...
    acc_together = cluster_acc_v1(fea, centers, labels_map)
    loss = clusterloss(fea, labels_map, centers, lr, iteration)
    # pred = nn.Sigmoid()(pred)
    ...
    return loss, acc_seperate, acc_together
...
def validation_routine(net, val_loader, tag: str, iteration, tb, loader_len_norm: int = None):
    ...
    for val_data in tqdm(val_loader, desc='Validation', leave=False, total=len(val_loader)):
        ...
        with torch.no_grad():
            val_batch_loss, val_batch_pred, val_batch_labels = batch_forward(net, batch_data,
