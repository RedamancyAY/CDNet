...
def dist_train(gpu, args):
    ...
    load_model_straight(
        model_t, optimizer, '/mnt/8T/hou/multicard_forTest/weights/binclass/net-CNet_traindb-ff-c23-720-140-140_face-scale_patchSize-299_cuttingsSize-64_seed-43_note-t18/it000020.pth')
    ...
    while epoch != epoch_run:
        ...
        for train_batch in tqdm(train_loader, desc='Epoch {:03d} '.format(epoch), leave=False,
            ...
            train_batch_pred, train_batch_loss, (T_c, F_c), batch_p = batch_forward(
                model_t, batch_data, batch_labels, batch_paths, (T_c, F_c))
...
def batch_forward(model: nn.Module, data: torch.Tensor, labels: torch.Tensor, paths: list[Path], centers):
    ...
    loss, centers = clusterloss(fea, labels, centers)
...
def clusterloss(fea: torch.Tensor, labels: torch.Tensor, centers):
    ...
    tmp1_0 = centerT-tmp1
    tmp1_1 = torch.sum(torch.mul(tmp1_0, tmp1_0), dim=1)-0.4*dis_c
    tmp1_1[tmp1_1 < 0] = 0

    loss_same = loss_same+torch.sum(tmp1_1)

    tmp2_0 = centerF-tmp2
    tmp2_1 = torch.sum(torch.mul(tmp2_0, tmp2_0), dim=1)-0.4*dis_c
    tmp2_1[tmp2_1 < 0] = 0

    loss_same = loss_same+torch.sum(tmp2_1)
    loss_same = loss_same/labels.size(0)

    loss_dif = 1/distance(avg1, avg2)

    centerT_1 = 0.9999*centerT+0.0001*avg1.detach()
    centerF_1 = 0.9999*centerF+0.0001*avg2.detach()
    ...
    for val_data in tqdm(val_loader, desc='{}_Validation'.format(tag), leave=False, total=len(val_loader)):
        ...
        with torch.no_grad():
            val_batch_pred, val_batch_loss, _, val_batch_p = batch_forward(net, batch_data,
                                                                           batch_labels, batch_paths, centers)
