...
def extract_box(frame: Image.Image, kpx: int, kpy: int, xlen: int, ylen: int):

    size = int(min(xlen, ylen)/6)
    left = kpx-size
    right = kpx+size
    top = kpy-size
    bottom = kpy+size
    box = frame.crop((left, top, right, bottom))
