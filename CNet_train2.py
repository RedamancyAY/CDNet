    ...
    while epoch != epoch_run:
        ...
        for train_batch in tqdm(train_loader, desc='Epoch {:03d} '.format(epoch), leave=False,
            ...
            train_batch_pred, train_batch_loss, (T_c, F_c), batch_p = batch_forward(
                model_t, batch_data, batch_labels, batch_paths, (T_c, F_c), mr, sr)
...
def batch_forward(model: nn.Module, data: torch.Tensor, labels: torch.Tensor, paths: list[Path], centers, mr, sr):
    ...
    loss, centers = clusterloss(fea, labels, centers, mr, sr)
...
def clusterloss(fea: torch.Tensor, labels: torch.Tensor, centers):
    ...
    tmp1_1 = torch.sum(torch.mul(tmp1_0, tmp1_0), dim=1) - \
        0.4*(0.5*distance(avg1, avg2)+0.5*dis_c)
    tmp1_1[tmp1_1 < 0] = 0
